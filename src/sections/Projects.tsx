      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-white">{isFullList ? t('projects.all_title') : t('projects.featured_title')}</h2>
              <p className="text-gray-400 mt-1">{isFullList ? t('projects.all_desc') : t('projects.featured_desc')}</p>
            </div>
          </div>
          {isFullList && (
            <button onClick={() => navigate('/')} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors">
              <ChevronLeft className="w-5 h-5" /> {t('projects.back_home')}
            </button>
          )}
        </div>

        {loading ? <div className="text-center py-20 animate-pulse text-gray-400">{t('projects.loading')}</div> : 
          <>
              ...
              {projects.map((project, index) => (
                <div key={project.id || index} onClick={() => setSelectedProject(project)} className="group bg-slate-800 rounded-xl overflow-hidden border border-white/5 hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 flex flex-col cursor-pointer">
                  <div className="relative h-48 overflow-hidden">
                    ...
                    {project.isFeatured && <div className="absolute top-2 right-2 bg-yellow-500/90 text-black text-xs font-bold px-2 py-1 rounded shadow-lg z-20">{t('projects.featured_badge')}</div>}
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {project.tags?.slice(0, 3).map((tag, i) => (
                        <span key={i} className="px-3 py-1 text-xs font-medium text-blue-300 bg-blue-500/10 rounded-full">{tag}</span>
                      ))}
                    </div>
                    <div className="flex items-center gap-2 text-blue-400 text-sm font-medium mt-auto group-hover:translate-x-2 transition-transform">
                      {t('projects.inspect')} <ExternalLink className="w-4 h-4" />
                    </div>
                  </div>
                </div>
              ))}
            ...
            {!isFullList && (
              <div className="mt-12 text-center">
                <button onClick={() => navigate('/all-projects')} className="inline-flex items-center gap-2 px-8 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-full font-medium transition-all border border-white/10 group">
                  {t('projects.view_all')} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            )}
          </>
      ...
      <div className="relative bg-slate-900 w-full max-w-6xl max-h-[95vh] overflow-y-auto rounded-2xl border border-white/10 shadow-2xl animate-in fade-in zoom-in duration-300 custom-scrollbar">
        ...
        <div className="px-6 md:px-10 pb-10 -mt-16">
          ...
          <div className="mb-10 shadow-2xl shadow-black/50 rounded-2xl">
            {project.gallery && project.gallery.length > 0 ? <ImageGallery images={project.gallery} /> : <div className="w-full h-[50vh] bg-slate-950 rounded-2xl overflow-hidden flex items-center justify-center border border-white/10"><img src={project.image} className="h-full object-contain" alt={project.title} /></div>}
          </div>
          <div className="grid md:grid-cols-3 gap-10 mb-10">
            <div className="md:col-span-2 space-y-4">
              <h3 className="text-xl font-bold text-white flex items-center gap-2 border-b border-white/10 pb-2"><Info className="w-5 h-5 text-blue-400" /> {t('projects.story')}</h3>
              <p className="text-gray-300 text-lg leading-relaxed whitespace-pre-line">{project.longDescription}</p>
            </div>
            <div className="space-y-4">
              <h3 className="text-xl font-bold text-white flex items-center gap-2 border-b border-white/10 pb-2"><Cpu className="w-5 h-5 text-purple-400" /> {t('projects.features')}</h3>
              <ul className="space-y-3">
                {project.features?.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3 text-gray-300 p-3 bg-slate-800/50 rounded-lg border border-white/5 hover:border-blue-500/30 transition-colors">
                    <span className="w-2 h-2 bg-blue-400 rounded-full mt-2 shrink-0 shadow-[0_0_8px_rgba(96,165,250,0.6)]"></span>
          ...
          <div className="pt-8 border-t border-white/10 flex flex-wrap justify-center gap-4">
            <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-8 py-4 rounded-xl transition-all font-semibold border border-white/10 hover:scale-105"><Github className="w-5 h-5" /> {t('projects.source_code')}</a>
            {project.liveUrl && (<a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-xl transition-all font-semibold shadow-lg shadow-green-900/30 hover:scale-105"><ExternalLink className="w-5 h-5" /> {t('projects.demo')}</a>)}
            {project.linkedinUrl && (<a href={project.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#0077b5] hover:bg-[#006097] text-white px-8 py-4 rounded-xl transition-all font-semibold shadow-lg shadow-blue-900/30 hover:scale-105"><Linkedin className="w-5 h-5" /> {t('projects.linkedin')}</a>)}
          </div>
        </div>
      </div>
    </div>
